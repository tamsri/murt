...
def TracerTest():
    ...
    for i in range(n):
        ...
        #    tx_pos = [np.random.random()*300-150, np.random.random() *
        #             4+1.5, np.random.random()*300-150]
        while a.is_outdoor(rx_pos) is not True:
            rx_pos = [np.random.random() * 300 - 150, np.random.random() +
                      1.5, np.random.random() * 300 - 150]
